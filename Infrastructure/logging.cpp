...
public:
    LoggingPreInitializer() {
        logger = spdlog::create<spdlog::sinks::null_sink_mt>("core");
    }
} loggingPreInitializer;
...
void InitLogging(const std::wstring &logFile, spdlog::level::level_enum logLevel) {
    ...
    try {
        ...
        auto fileSink = std::make_shared<spdlog::sinks::basic_file_sink_mt>(ucs2_to_local(logFile), true);
        ...
        std::vector<std::shared_ptr<spdlog::sinks::sink>> sinks{fileSink, debugSink};
        logger = spdlog::create<spdlog::sinks::dist_sink_mt>("core", sinks);
