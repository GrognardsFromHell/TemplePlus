...
void UiLegacyManager::SetHidden(LgcyWidgetId id, bool hidden)
{
	...
	// this might make hover-buttons etc. show the correct state based on the current mouse-pos
	/*
	mouseargs[0] = mousestate.x;
	mouseargs[3] = 0x1000;
	mouseargs[1] = mousestate.y;
	mouseargs[2] = mousestate.field_24;
	ui_widget_handle_mouse_msg((tig_msg_args *)mouseargs);
	tig_mouse_getstate(&mousestate);
	*/
}
