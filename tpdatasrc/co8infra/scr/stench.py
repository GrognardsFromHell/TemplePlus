...
def getStenchObj(critter):
	...
	while (stench_obj != OBJ_HANDLE_NULL and not is_spell_flag_set(stench_obj, OSF_IS_HEZROU_STENCH) and it < 217):
		stench_obj = critter.inventory_item(it)
		it += 1
	if (it >= 217):
		print 'Search for Stench Object a lot of times!'
	return stench_obj
...
def endStench(caster, spell_id):
	...
	if (caster != OBJ_HANDLE_NULL):
		...
		print 'Caster is null now, trying default'
		caster = game.leader # just to get a location
	for critter in game.obj_list_vicinity(caster.location, OLC_CRITTERS):
