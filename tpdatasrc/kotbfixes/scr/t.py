...
def tas():
	game.fade_and_teleport( 0,0,0,5131,480,480 )
...
def generate_roll_file():
	f = open('dice_roll_new.mes', 'w')
	roll_result = 0
	while roll_result < 20: # note: range(0,20) returns [0,1,..., 19], so I'll add +1 anyway
		skill_mod = -49
		while skill_mod < 50:
			if skill_mod < 0:
				f.write("{" + str( (roll_result+1)*10000 + skill_mod + 49 ) + "}{" +  str( (roll_result+1)) + " - " + str( (-skill_mod)) + " = " + str(roll_result+1 + skill_mod) + " }\n")
			else:
				f.write("{" + str( (roll_result+1)*10000 + skill_mod + 49 ) + "}{" +  str( (roll_result+1)) + " + " + str( ( skill_mod)) + " = " + str(roll_result+1 + skill_mod) + " }\n")
			skill_mod += 1
		roll_result+=1
		#+ skill_mod+49
				#f.write("{"  "}{" +  str(roll_result+1) + " - " + str(-skill_mod)" }\n")
			#else:
			#	f.write("{" + str(1000*roll_result + (skill_mod-GEN_skill_mod_MIN) ) + "}{" +  str(roll_result+1) + " + " + str( skill_mod)" }\n")
	f.close()
...
def gimme(gversion = 1, give_earth = 0, give_air = 0, give_water = 0):
