...
def OnBeginSpellCast(spell):
    print "Marshal Minor Aura OnBeginSpellCast"
    print "spell.target_list=", spell.target_list
    print "spell.caster=", spell.caster, " caster.level= ", spell.caster_level

def OnSpellEffect(spell):
    print "Marshal Minor Aura OnSpellEffect"
    print "spell.target_list=", spell.target_list
    print "spell.caster=", spell.caster, " caster.level= ", spell.caster_level
    ...
    spell.caster.condition_add_with_args("sp-Marshal Minor Aura", spell.id, auraEnum, auraEventId, aura_type_minor, 0)
    ...
    for spellTarget in spell.target_list:
        if spellTarget.obj.is_friendly(spell.caster) and aura_utils.verifyTarget(spellTarget.obj):
            spellTarget.obj.condition_add_with_args("Marshal Minor Aura {}".format(auraName), spell.id, auraEnum, auraEventId, 0, 0)
        else:
            targetsToRemove.append(spellTarget.obj)
    spell.target_list.remove_list(targetsToRemove)

def OnBeginRound(spell):
    print "Marshal Minor Aura OnBeginRound"

def OnEndSpellCast(spell):
    print "Marshal Minor Aura OnEndSpellCast"
