...
def OnSpellEffect ( spell ):
	...
	for target_item in list(spell.target_list):  # for target_item in spell.target_list: 
		...
		if not (alignment & ALIGNMENT_LAWFUL) and not (target_item.obj == spell.caster):

			game.particles( 'sp-Destroy Undead', target_item.obj )
			dbug("    target_item NOT LAWFUL",-99,'holy')
			...
			if obj_hit_dice <= (spell.caster_level - 1):
				...
				target_item.obj.condition_add_with_args( 'sp-Slow', spell.id, spell.duration, 1 )
...
def fix_caster_level (spell):	
	...
	if spell.caster.obj_get_int(obj_f_critter_domain_1) == law or spell.caster.obj_get_int(obj_f_critter_domain_2) == law:
