...
def OnSpellEffect ( spell ):
	...
	for target_item in list(spell.target_list):  # for target_item in spell.target_list: 
		...
		if not (alignment & ALIGNMENT_GOOD) and not (target_item.obj == spell.caster):
		
			game.particles( 'sp-Holy Smite', target_item.obj )
			dbug("    target_item NOT GOOD",-99,'holy')
...
def fix_caster_level (spell):	
	...
	dbug("BEFORE  spell.target_list",spell.target_list,'holy')	
	if spell.caster.obj_get_int(obj_f_critter_domain_1) == good or spell.caster.obj_get_int(obj_f_critter_domain_2) == good:
