...
public:
	...
	static int IsModifierActive(int dinputKey);
	static void Update();

	static int ToVirtualKey(int dinputKey);
...
int KeyboardFix::IsKeyPressed(int dinputKey) {
	
	auto virtualKey = ToVirtualKey(dinputKey);

	return infrastructure::gKeyboard.IsKeyPressed(virtualKey) ? 1 : 0;

}

int KeyboardFix::IsModifierActive(int dinputKey) {

	auto virtualKey = ToVirtualKey(dinputKey);

	return infrastructure::gKeyboard.IsModifierActive(virtualKey) ? 1 : 0;

}

void KeyboardFix::Update() {
	infrastructure::gKeyboard.Update();
}

// Translate dinput key to virtual key
int KeyboardFix::ToVirtualKey(int dinputKey) {
	switch (dinputKey) {
