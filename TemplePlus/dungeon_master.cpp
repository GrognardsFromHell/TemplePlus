...
void DungeonMaster::RenderDaynightXfer()
{
	...
	static std::map<ObjectId, std::string> nameCache;
	...
	while (dnx) {
		...
		if (ImGui::TreeNode(fmt::format("{}: {} ({})", idx++, id.ToString(), handle).c_str())) {
...
void DungeonMaster::RenderActiveSpells()
{
	...
	spellSys.DoForSpellsCastRegistry([&](SpellPacket& data) {
		...
		if (ImGui::TreeNode(fmt::format("ID {} | {} ({}) {}", pkt.spellId, spellSys.GetSpellName(pkt.spellEnum), pkt.spellEnum, isActive != 0 ? "" : "(inactive)").c_str())) {
			ImGui::Text(fmt::format("Caster: {}, CL {}, inv idx: {}", pkt.caster, pkt.casterLevel, pkt.invIdx != INV_IDX_INVALID ? (int) pkt.invIdx : -1).c_str());
			ImGui::Indent();
