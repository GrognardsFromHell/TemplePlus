...
QSGD3D11Renderer::QSGD3D11Renderer(QSGRenderContext *context)
    : QSGRenderer(context),
      m_renderList(16),
      m_vboData(1024),
      m_iboData(256),
...
QSGD3D11Renderer::~QSGD3D11Renderer()
{
    if (m_engine) {
...
void QSGD3D11Renderer::renderScene(GLuint fboId)
{
...
void QSGD3D11Renderer::updateMatrices(QSGNode *node, QSGTransformNode *xform)
{
    ...
    if (node->type() == QSGNode::TransformNodeType) {
        ...
        if (node->type() == QSGNode::GeometryNodeType || node->type() == QSGNode::ClipNodeType) {
            m_nodeDirtyMap[node] |= QSGD3D11MaterialRenderState::DirtyMatrix;
...
void QSGD3D11Renderer::updateOpacities(QSGNode *node, float inheritedOpacity)
{
    if (node->isSubtreeBlocked())
        return;
    ...
    if (node->type() == QSGNode::OpacityNodeType) {
        ...
        if (node->type() == QSGNode::GeometryNodeType) {
            m_nodeDirtyMap[node] |= QSGD3D11MaterialRenderState::DirtyOpacity;
...
void QSGD3D11Renderer::buildRenderList(QSGNode *node, QSGClipNode *clip)
{
    if (node->isSubtreeBlocked())
        return;
    ...
    if (node->type() == QSGNode::GeometryNodeType || node->type() == QSGNode::ClipNodeType) {
        ...
        if (node->type() == QSGNode::GeometryNodeType) {
            QSGD3D11Material *m = static_cast<QSGD3D11Material *>(static_cast<QSGGeometryNode *>(node)->activeMaterial());
            e.cboSize = m->constantBufferSize();
        } else {
            // Stencil-based clipping needs a 4x4 matrix.
            e.cboSize = QSGD3D11Engine::alignedConstantBufferSize(16 * sizeof(float));
        }
...
void QSGD3D11Renderer::render()
{
    QSGD3D11RenderContext *rc = static_cast<QSGD3D11RenderContext *>(context());
    m_engine = rc->engine();
    if (!m_layerRenderer)
        m_engine->beginFrame();
    else
...
void QSGD3D11Renderer::nodeChanged(QSGNode *node, QSGNode::DirtyState state)
{
...
void QSGD3D11Renderer::renderElements()
{
    ...
    m_engine->queueClearDepthStencil(1, 0, QSGD3D11Engine::ClearDepth | QSGD3D11Engine::ClearStencil);

    m_pipelineState.blend = m_freshPipelineState.blend = QSGD3D11PipelineState::BlendNone;
    ...
    m_pipelineState.blend = m_freshPipelineState.blend = QSGD3D11PipelineState::BlendPremul;
...
void QSGD3D11Renderer::renderElement(int elementIndex)
{
    ...
    QSGD3D11Material *m = static_cast<QSGD3D11Material *>(gn->activeMaterial());
    ...
    QSGD3D11MaterialRenderState::DirtyStates dirtyState = m_nodeDirtyMap.value(e.node);

    // After a rebuild everything in the cbuffer has to be updated.
    if (!e.cboPrepared) {
        e.cboPrepared = true;
        dirtyState = QSGD3D11MaterialRenderState::DirtyAll;
    }
    ...
    if (m_projectionChangedDueToDeviceSize)
        dirtyState |= QSGD3D11MaterialRenderState::DirtyMatrix;
    ...
    QSGD3D11Material::ExtraState extraState;
    QSGD3D11Material::UpdateResults updRes = m->updatePipeline(state(dirtyState),
                                                               &m_pipelineState,
                                                               &extraState,
                                                               cboPtr);

    if (updRes.testFlag(QSGD3D11Material::UpdatedConstantBuffer))
        m_engine->markBufferDirty(m_constantBuf, e.cboOffset, e.cboSize);

    if (updRes.testFlag(QSGD3D11Material::UpdatedBlendFactor))
    ...
    if (g->drawingMode() == QSGGeometry::DrawLineStrip || g->drawingMode() == QSGGeometry::DrawLines) {
        if (g->lineWidth() != 1.0f)
            qWarning("QSGD3D11Renderer: Line widths other than 1 are not supported by this renderer");
    } else if (g->drawingMode() == QSGGeometry::DrawPoints) {
        if (g->lineWidth() != 1.0f)
            qWarning("QSGD3D11Renderer: Point sprites are not supported by this renderer");
    }
...
void QSGD3D11Renderer::setInputLayout(const QSGGeometry *g, QSGD3D11PipelineState *pipelineState)
{
    ...
    for (int i = 0; i < g->attributeCount(); ++i) {
        QSGD3D11InputElement &ie(pipelineState->inputElements[i]);
        ...
        ie.offset = offset;
        int bytesPerTuple = 0;
        ie.format = QSGD3D11Engine::toDXGIFormat(QSGGeometry::Type(attrs[i].type), tupleSize, &bytesPerTuple);
        if (ie.format == FmtUnknown)
            qFatal("QSGD3D11Renderer: unsupported tuple size for attribute type 0x%x", attrs[i].type);
...
void QSGD3D11Renderer::queueDrawCall(const QSGGeometry *g, const QSGD3D11Renderer::Element &e)
{
    QSGD3D11Engine::DrawParams dp;
    ...
    if (e.iboOffset >= 0) {
        ...
        const QSGD3D11Format indexFormat = QSGD3D11Engine::toDXGIFormat(indexType);
        if (indexFormat == FmtUnknown)
            qFatal("QSGD3D11Renderer: unsupported index type 0x%x", indexType);
...
void QSGD3D11Renderer::setupClipping(const QSGClipNode *clip, int elementIndex)
{
    const QRect devRect = deviceRect();
    QRect scissorRect;
    int clipTypes = 0;
...
void QSGD3D11Renderer::setScissor(const QRect &r)
{
...
void QSGD3D11Renderer::renderStencilClip(const QSGClipNode *clip, int elementIndex,
                                         const QMatrix4x4 &m, quint32 &stencilValue)
{
    QSGD3D11PipelineState sps;
    ...
    m_engine->queueClearDepthStencil(1, 0, QSGD3D11Engine::ClearStencil);
    sps.stencilEnable = true;
    sps.colorWrite = false;
    sps.depthWrite = false;

    sps.stencilFunc = QSGD3D11PipelineState::CompareEqual;
    sps.stencilFailOp = QSGD3D11PipelineState::StencilKeep;
    sps.stencilDepthFailOp = QSGD3D11PipelineState::StencilKeep;
    sps.stencilPassOp = QSGD3D11PipelineState::StencilIncr;
...
void QSGD3D11Renderer::renderRenderNode(QSGRenderNode *node, int elementIndex)
{
