...
QSGD3D11Layer::QSGD3D11Layer(QSGD3D11RenderContext *rc)
    : m_rc(rc)
{
...
int QSGD3D11Layer::textureId() const
{
    return m_rt; // not a texture id per se but will do
}

QSize QSGD3D11Layer::textureSize() const
{
    return m_size;
}

bool QSGD3D11Layer::hasAlphaChannel() const
{
    return true;
}

bool QSGD3D11Layer::hasMipmaps() const
{
    // mipmapped layers are not supported for now
    return false;
}

QRectF QSGD3D11Layer::normalizedTextureSubRect() const
{
...
void QSGD3D11Layer::bind()
{
    if (Q_UNLIKELY(debug_render()))
        qDebug("layer %p bind rt=%u", this, m_rt);

    QSGD3D11Engine *engine = m_rc->engine();
    Q_ASSERT(m_rt);
...
bool QSGD3D11Layer::updateTexture()
{
    if (Q_UNLIKELY(debug_render()))
...
void QSGD3D11Layer::setItem(QSGNode *item)
{
    if (m_item == item)
        return;
...
void QSGD3D11Layer::setRect(const QRectF &rect)
{
    if (m_rect == rect)
        return;
...
void QSGD3D11Layer::setSize(const QSize &size)
{
    if (m_size == size)
        return;
...
void QSGD3D11Layer::scheduleUpdate()
{
    if (m_updateContentPending)
        return;
...
QImage QSGD3D11Layer::toImage() const
{
    return m_rc->engine()->executeAndWaitReadbackRenderTarget(m_rt);
}

void QSGD3D11Layer::setLive(bool live)
{
    if (m_live == live)
        return;
...
void QSGD3D11Layer::setRecursive(bool recursive)
{
    m_recursive = recursive;
}

void QSGD3D11Layer::setFormat(uint format)
{
    Q_UNUSED(format);
}

void QSGD3D11Layer::setHasMipmaps(bool mipmap)
{
    // mipmapped layers are not supported for now
    Q_UNUSED(mipmap);
}

void QSGD3D11Layer::setDevicePixelRatio(qreal ratio)
{
    m_dpr = ratio;
}

void QSGD3D11Layer::setMirrorHorizontal(bool mirror)
{
    m_mirrorHorizontal = mirror;
}

void QSGD3D11Layer::setMirrorVertical(bool mirror)
{
    m_mirrorVertical = mirror;
}

void QSGD3D11Layer::markDirtyTexture()
{
    if (Q_UNLIKELY(debug_render()))
...
void QSGD3D11Layer::invalidated()
{
    cleanup();
}

void QSGD3D11Layer::cleanup()
{
    if (!m_renderer && !m_rt)
        return;
...
void QSGD3D11Layer::resetRenderTarget()
{
    if (!m_rt)
        return;
...
void QSGD3D11Layer::updateContent()
{
    if (Q_UNLIKELY(debug_render()))
    ...
    if (!m_renderer) {
        ...
        static_cast<QSGD3D11Renderer *>(m_renderer)->turnToLayerRenderer();
        connect(m_renderer, &QSGRenderer::sceneGraphChanged, this, &QSGD3D11Layer::markDirtyTexture);
    }
    ...
    if (!m_rt || m_rtSize != m_size) {
        ...
        engine->createRenderTarget(m_rt, m_rtSize, sampleCount);
    ...
    if (m_recursive && !m_secondaryRT && sampleCount == 1) {
        ...
        engine->createRenderTarget(m_secondaryRT, m_rtSize, sampleCount);
    }
