...
QSGD3D11PainterTexture::QSGD3D11PainterTexture(QSGD3D11Engine *engine)
    : QSGD3D11Texture(engine)
{
}

void QSGD3D11PainterTexture::bind()
{
    if (m_image.isNull()) {
        if (!m_id) {
        ...
        m_id = m_engine->genTexture();
        m_engine->createTexture(m_id, m_image.size(), m_image.format(), QSGD3D11Engine::TextureWithAlpha);
...
QSGD3D11PainterNode::QSGD3D11PainterNode(QQuickPaintedItem *item)
    : m_item(item),
      m_engine(static_cast<QSGD3D11RenderContext *>(QQuickItemPrivate::get(item)->sceneGraphRenderContext())->engine()),
      m_texture(new QSGD3D11PainterTexture(m_engine)),
...
QSGD3D11PainterNode::~QSGD3D11PainterNode()
{
    delete m_texture;
}

void QSGD3D11PainterNode::setPreferredRenderTarget(QQuickPaintedItem::RenderTarget)
{
    // always QImage-based
}

void QSGD3D11PainterNode::setSize(const QSize &size)
{
    if (m_size == size)
        return;
...
void QSGD3D11PainterNode::setDirty(const QRect &dirtyRect)
{
...
void QSGD3D11PainterNode::setOpaquePainting(bool)
{
    // ignored
}

void QSGD3D11PainterNode::setLinearFiltering(bool linearFiltering)
{
    m_material.setFiltering(linearFiltering ? QSGTexture::Linear : QSGTexture::Nearest);
    markDirty(DirtyMaterial);
}

void QSGD3D11PainterNode::setMipmapping(bool)
{
    // ### not yet
}

void QSGD3D11PainterNode::setSmoothPainting(bool s)
{
...
void QSGD3D11PainterNode::setFillColor(const QColor &c)
{
    if (m_fillColor == c)
        return;
...
void QSGD3D11PainterNode::setContentsScale(qreal s)
{
...
void QSGD3D11PainterNode::setFastFBOResizing(bool)
{
    // nope
}

void QSGD3D11PainterNode::setTextureSize(const QSize &size)
{
...
QImage QSGD3D11PainterNode::toImage() const
{
    return *m_texture->image();
}

void QSGD3D11PainterNode::update()
{
    if (m_dirtyGeometry) {
        m_dirtyGeometry = false;
        QRectF src(0, 0, 1, 1);
...
QSGTexture *QSGD3D11PainterNode::texture() const
{
    return m_texture;
}
