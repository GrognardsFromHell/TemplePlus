...
#ifndef QSGD3D11ENGINE_P_H
#define QSGD3D11ENGINE_P_H
...
struct QSGD3D11InputElement
{
    const char *semanticName = nullptr;
    int semanticIndex = 0;
    QSGD3D11Format format = FmtFloat4;
    quint32 slot = 0;
    quint32 offset = 0;

    bool operator==(const QSGD3D11InputElement &other) const {
...
inline uint qHash(const QSGD3D11InputElement &key, uint seed = 0)
{
    return qHash(key.semanticName, seed) + key.semanticIndex + key.format + key.offset;
}

struct QSGD3D11TextureView
{
    enum Filter {
        FilterNearest = 0,
        FilterLinear = 0x15,
    ...
    bool operator==(const QSGD3D11TextureView &other) const {
...
inline uint qHash(const QSGD3D11TextureView &key, uint seed = 0)
{
    Q_UNUSED(seed);
    return key.filter + key.addressModeHoriz + key.addressModeVert;
}

const int QSGD3D11_MAX_TEXTURE_VIEWS = 8;

struct QSGD3D11RootSignature
{
    int textureViewCount = 0;
    QSGD3D11TextureView textureViews[QSGD3D11_MAX_TEXTURE_VIEWS];

    bool operator==(const QSGD3D11RootSignature &other) const {
...
inline uint qHash(const QSGD3D11RootSignature &key, uint seed = 0)
{
...
struct QSGD3D11ShaderState
{
    ...
    QSGD3D11RootSignature rootSig;

    bool operator==(const QSGD3D11ShaderState &other) const {
...
inline uint qHash(const QSGD3D11ShaderState &key, uint seed = 0)
{
    return qHash(key.vs, seed) + key.vsSize + qHash(key.ps, seed) + key.psSize + qHash(key.rootSig, seed);
}

const int QSGD3D11_MAX_INPUT_ELEMENTS = 8;

struct QSGD3D11PipelineState
{
    enum CullMode {
        CullNone = 1,
        CullFront,
    ...
    QSGD3D11ShaderState shaders;

    int inputElementCount = 0;
    QSGD3D11InputElement inputElements[QSGD3D11_MAX_INPUT_ELEMENTS];
...
inline uint qHash(const QSGD3D11PipelineState &key, uint seed = 0)
{
...
{
public:
    QSGD3D11Engine(ID3D11Device *device, ID3D11DeviceContext *context, IDXGISwapChain *swapChain);
    ~QSGD3D11Engine();

    bool attachToWindow(const QSize &size, float dpr, int surfaceFormatSamples, bool alpha);
    ...
    struct DrawParams {
        ...
        QSGD3D11Format indexFormat = FmtUnsignedShort;
    };
    ...
    static QSGD3D11Format toDXGIFormat(QSGGeometry::Type sgtype, int tupleSize = 1, int *size = nullptr);
...
private:
    QSGD3D11EnginePrivate *d;
    Q_DISABLE_COPY(QSGD3D11Engine)
};

Q_DECLARE_OPERATORS_FOR_FLAGS(QSGD3D11Engine::ClearFlags)
Q_DECLARE_OPERATORS_FOR_FLAGS(QSGD3D11Engine::TextureCreateFlags)
Q_DECLARE_OPERATORS_FOR_FLAGS(QSGD3D11Engine::TextureUploadFlags)

QT_END_NAMESPACE

#endif // QSGD3D11ENGINE_P_H
