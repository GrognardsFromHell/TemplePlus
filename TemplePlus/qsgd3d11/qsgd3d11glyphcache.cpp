...
QSGD3D11GlyphCache::QSGD3D11GlyphCache(QSGD3D11Engine *engine, QFontEngine::GlyphFormat format, const QTransform &matrix)
    : QTextureGlyphCache(format, matrix),
      m_engine(engine)
{
}

QSGD3D11GlyphCache::~QSGD3D11GlyphCache()
{
    if (m_id)
        m_engine->releaseTexture(m_id);
}

void QSGD3D11GlyphCache::createTextureData(int width, int height)
{
    ...
    m_engine->createTexture(m_id, m_size, imageFormat, QSGD3D11Engine::TextureWithAlpha
#ifdef ALWAYS_32BIT
                            | QSGD3D11Engine::TextureAlways32Bit
#endif
                            );
}

void QSGD3D11GlyphCache::resizeTextureData(int width, int height)
{
    width = qMax(128, width);
    height = qMax(32, height);
...
void QSGD3D11GlyphCache::beginFillTexture()
{
    Q_ASSERT(m_glyphImages.isEmpty() && m_glyphPos.isEmpty());
}

void QSGD3D11GlyphCache::fillTexture(const Coord &c, glyph_t glyph, QFixed subPixelPosition)
{
...
void QSGD3D11GlyphCache::endFillTexture()
{
...
#ifdef ALWAYS_32BIT
                                 , QSGD3D11Engine::TextureUploadAlways32Bit
#endif
                                 );
...
int QSGD3D11GlyphCache::glyphPadding() const
{
    return 1;
}

int QSGD3D11GlyphCache::maxTextureWidth() const
{
    return 16384;
}

int QSGD3D11GlyphCache::maxTextureHeight() const
{
    return 16384;
}

void QSGD3D11GlyphCache::useTexture()
{
    if (m_id)
        m_engine->useTexture(m_id);
}

QSize QSGD3D11GlyphCache::currentSize() const
{
    return m_size;
}
