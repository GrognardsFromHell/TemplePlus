...
void QSGD3D11Texture::create(const QImage &image, uint flags)
{
    // ### atlas?
...
int QSGD3D11Texture::textureId() const
{
    return m_id;
}

QSize QSGD3D11Texture::textureSize() const
{
    return m_image.size();
}

bool QSGD3D11Texture::hasAlphaChannel() const
{
    return m_alphaWanted;
}

bool QSGD3D11Texture::hasMipmaps() const
{
    return mipmapFiltering() != QSGTexture::None;
}

QRectF QSGD3D11Texture::normalizedTextureSubRect() const
{
    return QRectF(0, 0, 1, 1);
}

void QSGD3D11Texture::bind()
{
    ...
    if (m_createPending) {
        ...
        QSGD3D11Engine::TextureCreateFlags createFlags = 0;
        if (m_alphaWanted)
            createFlags |= QSGD3D11Engine::TextureWithAlpha;

        m_createdWithMipMaps = hasMipmaps();
        if (m_createdWithMipMaps)
            createFlags |= QSGD3D11Engine::TextureWithMipMaps;
