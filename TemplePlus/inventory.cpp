...
gfx::WeaponAnimType InventorySystem::GetWeaponAnimId(objHndl item, objHndl wielder){
	...
	if (IsWieldedTwoHanded(item, wielder)) {
		return (gfx::WeaponAnimType)temple::GetRef<int[74]>(0x102BE668)[objects.getInt32(item, obj_f_weapon_type)]; // two handed anims
	} else {
		return (gfx::WeaponAnimType)temple::GetRef<int[74]>(0x102BE540)[objects.getInt32(item, obj_f_weapon_type)]; // single handed anims
	}
}
