...
int InventorySystem::GetInventory(objHndl obj, objHndl ** inventoryArray)
{
	*inventoryArray = nullptr;
	...
	auto numItems = objects.getInt32(obj, invenNumField);
	if (numItems <= 0) return 0;
	*inventoryArray = static_cast<objHndl*>(malloc(sizeof(objHndl)*numItems));
	for (int i = 0; i < numItems; i++)
	{
		(*inventoryArray)[i] = objects.GetArrayFieldObjHndl(obj, invenField, i);
	}
	return numItems;
}
...
bool InventorySystem::IsItemNonTransferable(objHndl item, objHndl receiver)
{
	...
	if ((itemFlags & OIF_NO_DROP)
		&& (!receiver || !parent || objects.GetProtoNum(item) == 6239 || receiver != parent))
		return 7;
	if ( (itemFlags & OIF_NO_TRANSFER )
		&& receiver != parent && parent && !objects.IsDeadNullDestroyed(parent))
		return 8;
