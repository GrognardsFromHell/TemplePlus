...
std::optional<AnimSlotId> AnimSystem::GetFirstRunSlotId(objHndl handle) const {
	...
	for (auto slotIdx = GetFirstRunSlotIdxForObj(handle); 
		...
		auto goal = mGoals[slot.goals[0].goalType];
	...
	for (auto &slot : mSlots) {
		...
		if (slot.IsActive()) {
			if (gameSystems->GetTeleport().IsObjectTeleporting(slot.animObj)
				&& mGoals[slot.goals[0].goalType]->field_C) {
				SaveSlot(slot, fh);
				slotCount++;
			}
...
BOOL AnimSystem::PushGoalImpl(AnimSlotGoalStackEntry * gdata, AnimSlotId * slotId, int a3, int flags)
{
			...
			if (runInfo->goals[curGoal].goalType == ag_anim_idle) {
				...
				if (curGoal >= 7) {
					...
					*runInfo->pCurrentGoal = *gdata;
					...
					auto &goalState = runInfo->goals[runInfo->currentGoal];
					goalState.selfTracking = FrozenObjRef::Freeze(goalState.self.obj);
					goalState.targetTracking = FrozenObjRef::Freeze(goalState.target.obj);
					goalState.blockTracking = FrozenObjRef::Freeze(goalState.block.obj);
					goalState.scratchTracking = FrozenObjRef::Freeze(goalState.scratch.obj);
					goalState.parentTracking = FrozenObjRef::Freeze(goalState.parent.obj);
	...
	*runInfo->pCurrentGoal = *gdata;
	...
	auto &goalState = *runInfo->pCurrentGoal;
	goalState.selfTracking = FrozenObjRef::Freeze(goalState.self.obj);
	goalState.targetTracking = FrozenObjRef::Freeze(goalState.target.obj);
	goalState.blockTracking = FrozenObjRef::Freeze(goalState.block.obj);
	goalState.scratchTracking = FrozenObjRef::Freeze(goalState.scratch.obj);
