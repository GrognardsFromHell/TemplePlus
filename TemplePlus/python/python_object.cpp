...
static PyObject * PyObjHandle_PCAdd(PyObject* obj, PyObject* args) {
	...
	objHndl dude;
	if (!PyArg_ParseTuple(args, "O&:objhndl.pc_add", &ConvertObjHndl, &dude)) {
		return 0;
	}
	auto result = party.AddToPCGroup(dude);
	uiSystems->GetParty().Update();
	return PyInt_FromLong(result);
};
...
static PyObject * PyObjHandle_MakeClass(PyObject* obj, PyObject* args) {
	...
	auto gameObj = objSystem->GetObject(self->handle);
	if (level <= 0 || !gameObj->IsCritter() 
		|| ((uint32_t)level > config.maxLevel && gameObj->IsPC())
		|| ((uint32_t)level > XPTABLE_MAXLEVEL - 3 && gameObj->IsNPC())
	)
	{
		auto msg = gameObj->IsPC() ? format("Level {} is invalid for PC. Max {} (config.maxLevel), min 1.", level, config.maxLevel)
				: format("Level {} is invalid for NPC. Max {} (XPTABLE_MAXLEVEL-3), min 1.", level, XPTABLE_MAXLEVEL - 3);
