...
namespace temple {
    ...
    */
    ...
    std::wstring Dll::FindConflictingModule() {
        ...
        const uint32_t templeDesiredEnd = defaultBaseAddr + defaultImageSize;
        ...
        if (EnumProcessModules(hProcess, hMods, sizeof(hMods), &cbNeeded)) {
            for (uint32_t i = 0; i < (cbNeeded / sizeof(HMODULE)); i++) {
                ...
                if (fromAddress <= templeDesiredEnd && toAddress > defaultImageSize) {
    ...
    void Dll::SetReservedMemory(void *reservedMem, size_t reservedMemSize) {
        if (mReservedMem) {
            throw TempleException("Memory has already been reserved.");
        }
        if (reservedMemSize != defaultImageSize) {
            throw TempleException("Reserved memory size {:x} doesn't match default size {:x}",
                                  reservedMemSize, defaultImageSize);
        }
        if (mImpl) {
            throw TempleException("DLL has already been loaded.");
        }
